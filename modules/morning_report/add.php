<?php
// ================================================================
// FILE: C:\xampp\htdocs\wakala_system\modules\morning_report\add.php
// WAKALA FINANCIAL SYSTEM - ADD MORNING REPORT (FIXED)
// ================================================================
...
// ============================================================
// CHECK IF MORNING REPORT ALREADY EXISTS FOR TODAY
// ============================================================
$today = date('Y-m-d');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // DEBUG: See what is being posted
    ...
    foreach ($providers as $provider) {
        ...
        $provider_data[$code] = $amount;
        $total += $amount;
    }
    
    // DEBUG: Check collected data
    $debug_info .= '<pre>Provider Data: ' . print_r($provider_data, true) . '</pre>';
    $debug_info .= '<pre>Cash Balance: ' . $cash_balance . '</pre>';
    $debug_info .= '<pre>Total: ' . $total . '</pre>';
    ...
    if ($cash_balance <= 0 && !$has_value) {
        ...
        try {
            ...
            $report_number = generateNumber('MR');
            
            // Convert provider data to JSON
            $provider_json = json_encode($provider_data);
            
            // DEBUG: Check data before insert
            $debug_info .= '<pre>Report Number: ' . $report_number . '</pre>';
            $debug_info .= '<pre>Provider JSON: ' . $provider_json . '</pre>';
            
            // ============================================================
            // FIXED: INSERT INTO DATABASE - Use correct column names
            // ============================================================
            $stmt = $db->prepare("
            ...
            $debug_info .= '<pre>Insert Result: ' . ($result ? 'TRUE' : 'FALSE') . '</pre>';
            $debug_info .= '<pre>Last Insert ID: ' . $db->lastInsertId() . '</pre>';
            
            if ($result) {
                $report_id = $db->lastInsertId();
                
                // Log activity
                logActivity($employee_id, 'Add Morning Report', 'Morning Report', $report_id, '', 'New morning report added');
                
                $success = 'Morning Report submitted successfully! Report Number: ' . $report_number;
                
                // Redirect after 2 seconds
                echo '<meta http-equiv="refresh" content="2;url=view.php?id=' . $report_id . '">';
            } else {
                $error = 'Failed to insert data into database. Please check your input.';
                $debug_info .= '<pre>PDO Error Info: ' . print_r($stmt->errorInfo(), true) . '</pre>';
            }
            
        } catch (PDOException $e) {
            $error = 'Database error: ' . $e->getMessage();
            $debug_info .= '<pre>Exception: ' . $e->getMessage() . '</pre>';
            $debug_info .= '<pre>Stack Trace: ' . $e->getTraceAsString() . '</pre>';
        }
    }
}
...
<head>
    ...
    <title>Add Morning Report - Wakala</title>
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <div>
                    <h1>
                        <i class="fas fa-plus-circle" style="color:#DC2626;"></i> Add Morning Report
                        <small>Record morning float and cash balances for <?php echo date('d M Y'); ?></small>
                    </h1>
                </div>
            ...
            <div class="form-container">
                <form method="POST" action="" id="morningReportForm" autocomplete="off">
                    
                    <!-- Provider Fields -->
                    <div class="form-group">
                        <label>Provider Balances <span class="required">*</span></label>
                        <div class="form-text">Enter the opening float balance (OFB) for each provider</div>
                        ...
                        <div class="provider-grid">
                            <?php foreach ($providers as $provider): ?>
                                <div class="provider-item">
                                    ...
                                    <input type="text" 
                                           name="provider_<?php echo $provider['provider_code']; ?>" 
                                           id="provider_<?php echo $provider['provider_code']; ?>"
                                           class="form-control provider-input amount-input"
                                           placeholder="0"
                                           value="<?php echo isset($_POST['provider_' . $provider['provider_code']]) ? htmlspecialchars($_POST['provider_' . $provider['provider_code']]) : ''; ?>">
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               value="<?php echo isset($_POST['cash_balance']) ? htmlspecialchars($_POST['cash_balance']) : ''; ?>"
                               required>
                    ...
                    <div class="total-box">
                        ...
                        <span class="total-value" id="totalDisplay">TSh 0</span>
                    </div>
                    
                    <!-- Notes -->
                    <div class="form-group" style="margin-top:16px;">
                        <label for="notes">Notes</label>
                        <textarea name="notes" id="notes" class="form-control" placeholder="Additional notes (optional)"><?php echo isset($_POST['notes']) ? htmlspecialchars($_POST['notes']) : ''; ?></textarea>
                    </div>
                    
                    <!-- Form Actions -->
                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary btn-lg" id="submitBtn">
                            <i class="fas fa-save"></i> Submit Morning Report
                        </button>
                        <a href="index.php" class="btn btn-secondary">
    ...
    <script>
        ...
        // ============================================================
        function resetForm() {
            document.getElementById('morningReportForm').reset();
            setTimeout(function() {
                document.querySelectorAll('.provider-input').forEach(input => {
                    input.value = '';
                    input.dataset.rawValue = '';
                });
                const cash = document.getElementById('cash_balance');
                cash.value = '';
                cash.dataset.rawValue = '';
                calculateTotal();
            }, 100);
        }
        ...
        // FORM VALIDATION
        // ============================================================
        document.getElementById('morningReportForm').addEventListener('submit', function(e) {
            const providerInputs = document.querySelectorAll('.provider-input');
            const cashBalance = document.getElementById('cash_balance');
            let hasValue = false;
        ...
        console.log('%c MORNING REPORT - ADD FORM v2.0 (FIXED) ',
